    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "6px" }}>
          <div style={{
            ...
            background: "#00ff9d", animation: "pulse-dot 2s infinite"
          }} />
          <span style={{ fontSize: "11px", color: "#4a6a8a", letterSpacing: "3px", textTransform: "uppercase" }}>
            Personalized for C++ · 1–2 hrs/day · Deep Learning Mode
          </span>
        </div>
        <h1 style={{
          ...
          fontWeight: 800,
          margin: 0,
          background: "linear-gradient(90deg, #ffffff 0%, #8ab4d4 100%)",
        ...
        <p style={{ margin: "8px 0 0", color: "#4a7a9b", fontSize: "13px" }}>
          26 weeks · 5 phases · 150+ curated problems · Built like a 10-year MNC veteran would
        </p>
      </div>

      {/* Tabs */}
      <div style={{ display: "flex", borderBottom: "1px solid #1a2235", padding: "0 32px" }}>
        {["roadmap", "schedule", "c++ tips"].map(tab => (
          <button key={tab} className="tab-btn" onClick={() => setActiveTab(tab)} style={{
            ...
            letterSpacing: "2px", textTransform: "uppercase", padding: "14px 20px 12px",
      ...
      {activeTab === "roadmap" && (
        <div style={{ display: "flex", minHeight: "calc(100vh - 160px)" }}>
          {/* Left: Phase selector */}
          <div style={{
            width: "200px", minWidth: "200px", borderRight: "1px solid #1a2235",
            padding: "24px 16px", display: "flex", flexDirection: "column", gap: "8px",
          }}>
            <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "8px" }}>PHASES</div>
            {phases.map(p => (
              <button key={p.id} className="phase-btn" onClick={() => { setActivePhase(p.id); setActiveTopic(0); }}
                style={{
                ...
                <div style={{ fontSize: "10px", color: p.color, fontFamily: "'IBM Plex Mono'", fontWeight: 700, letterSpacing: "2px" }}>{p.title}</div>
                <div style={{ fontSize: "11px", color: activePhase === p.id ? "#c8d8e8" : "#4a6a8a", marginTop: "3px", lineHeight: 1.3 }}>{p.subtitle}</div>
                <div style={{ fontSize: "10px", color: "#2a4a6a", marginTop: "5px" }}>{p.duration}</div>
              </button>
            ))}
          </div>

          {/* Center: Topic list */}
          <div style={{ width: "220px", minWidth: "220px", borderRight: "1px solid #1a2235", padding: "24px 16px" }}>
            <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "12px" }}>TOPICS</div>
            {phase.topics.map((t, i) => (
              <button key={i} className="topic-btn" onClick={() => setActiveTopic(i)} style={{
                background: activeTopic === i ? "rgba(255,255,255,0.06)" : "transparent",
                border: `1px solid ${activeTopic === i ? "#2a3a50" : "transparent"}`,
                borderRadius: "8px", padding: "12px", cursor: "pointer", textAlign: "left",
                width: "100%", marginBottom: "6px", transition: "all 0.2s",
              }}>
                <div style={{ fontSize: "12px", color: activeTopic === i ? "#e8eaf0" : "#6a8aaa", fontWeight: activeTopic === i ? 600 : 400, lineHeight: 1.4 }}>{t.name}</div>
                <div style={{ fontSize: "10px", color: "#2a4a6a", marginTop: "4px" }}>{t.weeks}</div>
              </button>
            ))}
            <div style={{
              marginTop: "20px", padding: "12px", background: "#0d1a28",
              borderRadius: "8px", border: "1px solid #1a2235",
            }}>
              <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "2px", marginBottom: "8px" }}>MILESTONE</div>
              <div style={{ fontSize: "11px", color: "#4a7a6a", lineHeight: 1.6 }}>{phase.milestone}</div>
            </div>
          </div>

          {/* Right: Topic detail */}
          <div className="fade-in" key={`${activePhase}-${activeTopic}`} style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "8px" }}>
              <div>
                <div style={{ fontSize: "10px", color: phase.color, letterSpacing: "3px", marginBottom: "6px" }}>{topic.weeks.toUpperCase()}</div>
                <h2 style={{ fontFamily: "'Syne', sans-serif", fontSize: "22px", fontWeight: 800, margin: 0, color: "#ffffff" }}>{topic.name}</h2>
              </div>
            </div>

            <div style={{ background: "#0d1a28", border: `1px solid ${phase.color}30`, borderLeft: `3px solid ${phase.color}`, borderRadius: "8px", padding: "14px 18px", margin: "16px 0", fontSize: "13px", color: "#8ab4d4", lineHeight: 1.7 }}>
              <strong style={{ color: "#c8d8e8" }}>Why this matters: </strong>{topic.why}
            </div>

            {/* Concepts */}
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "12px" }}>CONCEPTS TO MASTER</div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                {topic.concepts.map((c, i) => (
                  <span key={i} style={{
                    background: "#0d1a28", border: "1px solid #1e3050",
                    borderRadius: "6px", padding: "6px 12px", fontSize: "12px", color: "#8ab4d4",
                  }}>{c}</span>
                ))}
              </div>
            </div>
            ...
            {/* Problems */}
            <div style={{ marginBottom: "24px" }}>
              <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "12px" }}>CURATED PROBLEMS</div>
                ...
                {topic.problems.map((p, i) => (
                  <a key={i} href={p.link} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                    <div className="prob-row" style={{
                      ...
                      padding: "13px 18px", borderBottom: i < topic.problems.length - 1 ? "1px solid #1a2235" : "none",
                      transition: "background 0.15s",
                    }}>
                      <span style={{ fontSize: "13px", color: "#c8d8e8" }}>{i + 1}. {p.name}</span>
                      <span style={{
                        fontSize: "10px", fontWeight: 700, color: diffColor[p.diff],
                        letterSpacing: "1px", background: `${diffColor[p.diff]}15`,
                        padding: "3px 10px", borderRadius: "4px",
                      }}>{p.diff.toUpperCase()}</span>
                    </div>
                  </a>
                ))}
              </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
              <div style={{ background: "#0d1a28", border: "1px solid #1e3050", borderRadius: "10px", padding: "18px" }}>
                <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "10px" }}>EXPERT TIP</div>
                <div style={{ fontSize: "12px", color: "#8ab4d4", lineHeight: 1.8 }}>{topic.tip}</div>
              </div>
              <div style={{ background: "#0d1a28", border: "1px solid #1e3050", borderRadius: "10px", padding: "18px" }}>
                <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "10px" }}>DAILY STRUCTURE</div>
      ...
      {activeTab === "schedule" && (
        <div style={{ padding: "32px", maxWidth: "800px" }}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "22px", marginBottom: "6px" }}>Weekly Schedule Template</h2>
          <p style={{ color: "#4a7a9b", fontSize: "13px", marginBottom: "28px" }}>1–2 hrs/day · Consistent beats intense · Applies to every phase</p>
          <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            {weeklySchedule.map((d, i) => (
              <div key={i} style={{
                display: "flex", alignItems: "center", gap: "20px",
                background: "#0d1220", border: "1px solid #1a2235",
                borderRadius: "10px", padding: "16px 20px",
              }}>
                <div style={{ minWidth: "40px", fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "14px", color: i === 6 ? "#4a6a8a" : "#e8eaf0" }}>{d.day}</div>
                <div style={{ fontSize: "20px" }}>{d.icon}</div>
                <div style={{ fontSize: "13px", color: i === 6 ? "#4a6a8a" : "#8ab4d4", lineHeight: 1.5 }}>{d.activity}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: "28px", background: "#0d1a28", border: "1px solid #1e3050", borderRadius: "12px", padding: "22px" }}>
            <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "14px" }}>THE GOLDEN RULE FROM 10 YEARS OF TEACHING DSA</div>
            <div style={{ fontSize: "14px", color: "#c8d8e8", lineHeight: 1.9 }}>
              <span style={{ color: "#00ff9d", fontWeight: 700 }}>Never</span> move to the next topic until you can solve 2 easy + 2 medium problems in that topic <span style={{ color: "#ffb800" }}>without hints</span>. Speed matters less than depth at your stage. One topic truly mastered beats five topics half-understood.
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === "c++ tips" && (
        <div style={{ padding: "32px", maxWidth: "800px" }}>
          <h2 style={{ fontFamily: "'Syne', sans-serif", fontWeight: 800, fontSize: "22px", marginBottom: "6px" }}>C++ DSA Cheatsheet</h2>
          <p style={{ color: "#4a7a9b", fontSize: "13px", marginBottom: "28px" }}>Patterns & snippets every C++ competitive programmer must know cold</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            {cppTips.map((t, i) => (
              <div key={i} style={{ background: "#0d1220", border: "1px solid #1a2235", borderRadius: "10px", padding: "18px" }}>
                <div style={{ fontSize: "11px", color: "#4a7a9b", marginBottom: "10px" }}>{t.tip}</div>
                <code style={{ fontSize: "12px", color: "#00ff9d", background: "#060a10", padding: "8px 12px", borderRadius: "6px", display: "block" }}>{t.code}</code>
              </div>
            ))}
          </div>
          <div style={{ marginTop: "24px", background: "#0d1220", border: "1px solid #1a2235", borderRadius: "12px", padding: "22px" }}>
            <div style={{ fontSize: "10px", color: "#2a4a6a", letterSpacing: "3px", marginBottom: "14px" }}>RESOURCES (ALL FREE)</div>
            {[
              { name: "NeetCode 150", desc: "Best curated problem list with video solutions", url: "https://neetcode.io/practice" },
              { name: "CP-Algorithms", desc: "Deep C++ algorithm implementations with proofs", url: "https://cp-algorithms.com" },
              { name: "Striver's A2Z Sheet", desc: "Most comprehensive Indian MNC prep sheet", url: "https://takeuforward.org/strivers-a2z-dsa-course/strivers-a2z-dsa-course-sheet-2/" },
              { name: "LeetCode Discuss", desc: "Pattern breakdowns by top contributors", url: "https://leetcode.com/discuss/" },
            ].map((r, i) => (
              <a key={i} href={r.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none" }}>
                <div className="prob-row" style={{
                  display: "flex", justifyContent: "space-between", alignItems: "center",
                  padding: "12px 0", borderBottom: i < 3 ? "1px solid #1a2235" : "none",
                  transition: "background 0.15s",
                }}>
                  <div>
                    <div style={{ fontSize: "13px", color: "#c8d8e8", fontWeight: 600 }}>{r.name}</div>
                    <div style={{ fontSize: "11px", color: "#4a6a8a", marginTop: "3px" }}>{r.desc}</div>
                  </div>
